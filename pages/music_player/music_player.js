...
Page({
  ...
  onLoad(options) {
    ...
    // 播放歌曲
    playStore.dispatch('playSong', id);
    // 读取共享数据
    this.onStoreState();
  },
  // 切换播放状态
  onPlaySwitch(){
    playStore.dispatch("switchPlayState")
  },
  // 切换播放模式
  onChangeMode() {
  ...
  onStoreState() {
    playStore.onStates(['playSong', 'durationTime', 'currentTime'], (
      ...
      if (currentTime != undefined && !this.data.handChange) {
    ...
    playStore.onStates(['lyricList', 'lyric_t', 'lyric_i'], (states) => {
      ...
      console.log(Object.fromEntries(markState));
  ...
  sliderChange(e) {
    ...
    setTimeout(() => { console.log(audioContext.paused) }, 100);
